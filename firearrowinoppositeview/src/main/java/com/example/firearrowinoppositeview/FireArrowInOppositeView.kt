...
class FireArrowInOppositeView(ctx : Context) : View(ctx) {
    ...
    data class FireArrowInOpposite(var i : Int) {

        private var curr : FAIONode = FAIONode(0)
    ...
    data class Renderer(var view : FireArrowInOppositeView) {

        private val animator : Animator = Animator(view)
        private val faio : FireArrowInOpposite = FireArrowInOpposite(0)
        private val paint : Paint = Paint(Paint.ANTI_ALIAS_FLAG)

        fun render(canvas : Canvas) {
            canvas.drawColor(backColor)
            faio.draw(canvas, paint)
            animator.animate {
                faio.update {
                    animator.stop()
                }
